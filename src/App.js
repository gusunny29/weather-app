...
const App = () => {
    ...
    if (timeFilter !== 'All Day') {
      filteredData = filteredData.map(day => ({
        ...
        hours: day.hours.filter(hour => {
          const hourOfDay = parseInt(hour.datetime.substring(hour.datetime.indexOf('T') + 1, hour.datetime.indexOf(':')));
          switch (timeFilter) {
            case 'Morning':
  ...
  const fetchCityState = async ({location}) => {
    try {
      const response = await axios.get(
  ...
  return (
    <Container>
      ...
      {weatherData && <WeatherDisplay data={weatherData} location={cityState} selectedDay={selectedDay} />}
    </Container>
  );
};
